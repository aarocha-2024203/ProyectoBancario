...
import {
  getAccounts, getCards, getLoans,
  getCoins, getAccountLocks, getServices, getAccountStatements,
  createCoin, deleteCoin, toggleCardStatus,
  deleteAccountLock, createAccount, deleteAccount,
...
import CardsPageAdmin from './CardsPageAdmin';
import TransactionsPageAdmin from './TransactionsPageAdmin';
import LoansPageAdmin from './LoansPageAdmin';
...
/* ══════════════════════════════════
   SECCIÓN: Overview (conservada de compañeros)
══════════════════════════════════ */
const OverviewSection = () => {
  ...
  const stats = [
    ...
    { label:'Cuentas activas', value: la ? '...' : accounts.filter(a=>(a.status||a.Status||'').toLowerCase()==='activa').length, icon:<svg viewBox="0 0 24 24" fill="none" width="20" height="20"><path d="M3 9l9-7 9 7v11a2 2 0 01-2 2H5a2 2 0 01-2-2z" stroke="#c8a951" strokeWidth="1.5" strokeLinecap="round"/></svg> },
...
/* ══════════════════════════════════
   SECCIÓN: Usuarios (conservada de compañeros — intacta)
══════════════════════════════════ */
const UsersSection = () => {
  ...
  const getRole = (u) => u?.role || u?.UserRoles?.[0]?.Role?.Name || u?._fetchedRole || 'USER_ROLE';
  ...
  const handleRoleChange = async (id, currentRole) => {
    ...
    const currentUserInList = localUsers.find(u => u.id === currentUser?.id || u.username === currentUser?.username);
    ...
    setChanging(id);
    setLocalUsers(prev => prev.map(u => (u.id === id || u.Id === id) ? { ...u, role: newRole, _fetchedRole: newRole } : u));
    ...
    try {
      ...
      setLocalUsers(prev => prev.map(u => (u.id === id || u.Id === id) ? { ...u, role: currentRole, _fetchedRole: currentRole } : u));
  ...
  return (
    <div>
      <div className="page-header">
        <div><h1 className="page-title">Usuarios registrados</h1><p className="page-subtitle">Gestión y control de acceso de usuarios</p></div>
      </div>
      <div className="table-card">
        ...
        <table className="data-table">
          ...
          <tbody>
            {loading && localUsers.length === 0 ? <LoadingRows cols={6}/> : filtered.map((u, i) => {
              const id = u.id || u.Id;
                ...
                <tr key={id || i}>
                  <td>
                    <div style={{ display:'flex', alignItems:'center', gap:'.75rem' }}>
                      <div style={{ width:34, height:34, borderRadius:'50%', background:'linear-gradient(135deg,#8a7035,#c8a951)', display:'flex', alignItems:'center', justifyContent:'center', fontFamily:"'Cormorant Garamond',serif", fontSize:'.85rem', fontWeight:700, color:'#060810', flexShrink:0, overflow:'hidden' }}>
                        {hasAvatar ? <img src={avatar} alt="" style={{ width:'100%', height:'100%', objectFit:'cover' }} onError={e => e.target.style.display='none'}/> : initials}
                      </div>
                      <div>
                        <p style={{ color:'var(--white)', fontWeight:500, fontSize:'.88rem', lineHeight:1.2 }}>{u.name||'—'} {u.surname||''}</p>
                  ...
                  <td>
                    <div className="action-btns">
                      ...
                      <button className="btn-icon" style={{ width:'auto', padding:'0 8px', fontSize:'.7rem', color:'var(--gold-pure)' }} disabled={changing === id} onClick={() => handleRoleChange(id, role)} title={role === 'ADMIN_ROLE' ? 'Quitar admin' : 'Hacer admin'}>
            ...
            {!loading && filtered.length === 0 && localUsers.length === 0 && <EmptyState text="Sin usuarios registrados"/>}
          </tbody>
        </table>
      </div>
      ...
      {editModal && (
        <div className="modal-overlay" onClick={() => setEditModal(null)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <div className="modal-header"><span className="modal-title">Detalle del usuario</span><button className="modal-close" onClick={() => setEditModal(null)}>✕</button></div>
            <div className="modal-body">
              <div style={{ textAlign:'center', marginBottom:'1.25rem' }}>
                <div style={{ width:72, height:72, borderRadius:'50%', margin:'0 auto .75rem', background:'linear-gradient(135deg,#8a7035,#c8a951)', display:'flex', alignItems:'center', justifyContent:'center', fontFamily:"'Cormorant Garamond',serif", fontSize:'1.5rem', fontWeight:700, color:'#060810', overflow:'hidden' }}>
                  {editModal.profilePicture && !editModal.profilePicture.includes('default') ? <img src={editModal.profilePicture} alt="" style={{ width:'100%', height:'100%', objectFit:'cover' }}/> : `${(editModal.name||'U')[0]}${(editModal.surname||'')[0]||''}`.toUpperCase()}
                </div>
                <p style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:'1.1rem', color:'var(--white)', fontWeight:600 }}>{editModal.name} {editModal.surname}</p>
              ...
              <div style={{ marginTop:'1rem', paddingTop:'1rem', borderTop:'1px solid rgba(255,255,255,0.06)' }}>
                ...
                <div style={{ display:'flex', gap:'.75rem' }}>
                  {['USER_ROLE','ADMIN_ROLE'].map(r => {
                    ...
                    return (
                      <button key={r} disabled={isActive || changing === editModal.id} onClick={() => { handleRoleChange(editModal.id, current); }}
                        style={{ flex:1, padding:'.65rem', background: isActive ? 'rgba(200,169,81,0.15)' : 'rgba(255,255,255,0.03)', border: `1px solid ${isActive ? 'rgba(200,169,81,0.35)' : 'rgba(255,255,255,0.07)'}`, color: isActive ? 'var(--gold-pure)' : 'var(--muted)', borderRadius:8, fontFamily:"'Outfit',sans-serif", fontSize:'.8rem', cursor: isActive ? 'default' : 'pointer' }}>
                        {r === 'ADMIN_ROLE' ? 'Administrador' : 'Cliente'}{isActive && ' ✓'}
                      </button>
                    );
                  })}
                </div>
            ...
            <div className="modal-footer"><button className="btn-cancel" onClick={() => setEditModal(null)}>Cerrar</button></div>
          </div>
        </div>
      )}
    </div>
...
/* ══════════════════════════════════
   SECCIÓN: Cuentas (conservada de compañeros — intacta)
══════════════════════════════════ */
...
const AccountsSection = () => {
  ...
  const emptyForm = { accountType: 'ahorro', balance: '', openingDate: '', status: 'activa', dailyWithdrawalLimit: '', annualInterestRate: '', currencyCode: 'GTQ', userId: '', dpi: '', address: '', jobName: '', monthlyIncome: '', phone: '' };
  const [form, setForm] = useState(emptyForm);

  useEffect(() => { if (data.length > 0) setLocalData(data); }, [data]);

  const filtered = localData.filter(a =>
    `${a.accountNumber||''} ${a.accountType||''} ${a.status||''} ${a.userId||''}`.toLowerCase().includes(search.toLowerCase())
  );

  const openCreate = () => { setForm(emptyForm); setEditItem(null); setModal(true); };
  const openEdit = (a) => {
    setForm({ accountType: a.accountType||'ahorro', balance: a.balance||'', openingDate: a.openingDate?a.openingDate.slice(0,10):'', status: a.status||'activa', dailyWithdrawalLimit: a.dailyWithdrawalLimit||'', annualInterestRate: a.annualInterestRate||'', currencyCode: a.currencyCode||'GTQ', userId: a.userId||'', dpi: a.dpi||'', address: a.address||'', name: a.name||'', jobName: a.jobName||'', monthlyIncome: a.monthlyIncome||'', phone: a.phone||'' });
  ...
  const handleSave = async () => {
    ...
    try {
      if (editItem) {
        await updateAccount(editItem.accountNumber, { name: form.name, address: form.address, jobName: form.jobName, monthlyIncome: Number(form.monthlyIncome) });
        setLocalData(prev => prev.map(a => a.accountNumber === editItem.accountNumber ? { ...a, ...form } : a));
        showSuccess('Cuenta actualizada');
      } else {
        const res = await createAccount({ userId: form.userId, currencyCode: form.currencyCode, monthlyIncome: Number(form.monthlyIncome), address: form.address, jobName: form.jobName, phone: form.phone, dpi: form.dpi, accountType: form.accountType, balance: Number(form.balance)||0, openingDate: form.openingDate?new Date(form.openingDate).toISOString():new Date().toISOString(), status: form.status, dailyWithdrawalLimit: Number(form.dailyWithdrawalLimit)||1000, annualInterestRate: Number(form.annualInterestRate)||0 });
        const newAccount = res.data?.data || res.data;
        setLocalData(prev => [newAccount, ...prev]);
        showSuccess('Cuenta creada exitosamente');
      }
      setModal(false); clearDataCache();
    } catch (e) { showError(e?.response?.data?.message || 'Error al guardar'); }
    finally { setSaving(false); }
  };

  const handleDelete = async () => {
    try { await deleteAccount(confirm.accountNumber); setLocalData(prev => prev.filter(a => a.accountNumber !== confirm.accountNumber)); showSuccess('Cuenta eliminada'); }
  ...
  const handleToggleStatus = async (a) => {
    ...
    try { await toggleAccountStatus(a.accountNumber, newStatus); setLocalData(prev => prev.map(x => x.accountNumber === a.accountNumber ? { ...x, status: newStatus } : x)); showSuccess(`Cuenta ${newStatus}`); }
  ...
  return (
    <div>
      <div className="page-header">
        ...
        <button className="btn-add" onClick={openCreate}><svg viewBox="0 0 24 24" fill="none" width="14" height="14"><path d="M12 5v14M5 12h14" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/></svg>Nueva cuenta</button>
      </div>
      ...
      <div className="table-card">
        ...
        <table className="data-table">
          <thead><tr><th>N° Cuenta</th><th>Tipo</th><th>Titular</th><th>Usuario ID</th><th>Balance</th><th>Moneda</th><th>Estado</th><th>Apertura</th><th>Acciones</th></tr></thead>
          <tbody>
            {loading && localData.length===0 ? <LoadingRows cols={9}/> : filtered.map((a,i) => (
              <tr key={a.accountNumber||i}>
                ...
                <td>
                  <div className="action-btns">
                    <button className="btn-icon" title="Ver detalle" onClick={()=>setDetailModal(a)}><svg viewBox="0 0 24 24" fill="none" width="13" height="13"><circle cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="1.5"/><path d="M12 8v4M12 16h.01" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/></svg></button>
                    <button className="btn-icon" title="Editar" onClick={()=>openEdit(a)}><svg viewBox="0 0 24 24" fill="none" width="13" height="13"><path d="M11 4H4a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2v-7" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/><path d="M18.5 2.5a2.121 2.121 0 013 3L12 15l-4 1 1-4 9.5-9.5z" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/></svg></button>
                    <button className="btn-icon" title={(a.status||'')==='activa'?'Desactivar':'Activar'} onClick={()=>handleToggleStatus(a)}>
                      {(a.status||'')==='activa'
                    ...
                    <button className="btn-icon danger" title="Eliminar" onClick={()=>setConfirm(a)}><svg viewBox="0 0 24 24" fill="none" width="13" height="13"><path d="M3 6h18M19 6l-1 14H6L5 6M10 11v6M14 11v6M9 6V4h6v2" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg></button>
                  </div>
                </td>
              </tr>
            ))}
      ...
      {modal && (
        <div className="modal-overlay" onClick={()=>setModal(false)}>
          <div className="modal" style={{maxWidth:560}} onClick={e=>e.stopPropagation()}>
            <div className="modal-header"><span className="modal-title">{editItem ? 'Editar cuenta' : 'Nueva cuenta'}</span><button className="modal-close" onClick={()=>setModal(false)}>✕</button></div>
            <div className="modal-body">
              {!editItem && (<>
                <div className="modal-fields-row">
                  <AccountField label="ID de usuario *"><input className="modal-input" placeholder="usr_XXXX" value={form.userId} onChange={e=>setForm(p=>({...p,userId:e.target.value}))}/></AccountField>
                  <AccountField label="Moneda *"><select className="modal-select" value={form.currencyCode} onChange={e=>setForm(p=>({...p,currencyCode:e.target.value}))}><option value="GTQ">GTQ — Quetzal</option><option value="USD">USD — Dólar</option><option value="EUR">EUR — Euro</option></select></AccountField>
                </div>
                <div className="modal-fields-row">
                  <AccountField label="Ocupación *"><input className="modal-input" placeholder="Ingeniero..." value={form.jobName} onChange={e=>setForm(p=>({...p,jobName:e.target.value}))}/></AccountField>
                  <AccountField label="Ingreso mensual (Q) *"><input className="modal-input" type="number" placeholder="5000" value={form.monthlyIncome} onChange={e=>setForm(p=>({...p,monthlyIncome:e.target.value}))}/></AccountField>
                </div>
                <AccountField label="Dirección *"><input className="modal-input" placeholder="Zona 10, Ciudad de Guatemala" value={form.address} onChange={e=>setForm(p=>({...p,address:e.target.value}))}/></AccountField>
                <div className="modal-fields-row">
                  <AccountField label="DPI (13 dígitos) *"><input className="modal-input" placeholder="1234567890123" maxLength={13} value={form.dpi} onChange={e=>setForm(p=>({...p,dpi:e.target.value.replace(/\D/g,'')}))}/></AccountField>
                  <AccountField label="Teléfono (8 dígitos) *"><input className="modal-input" placeholder="55123456" maxLength={8} value={form.phone} onChange={e=>setForm(p=>({...p,phone:e.target.value.replace(/\D/g,'')}))}/></AccountField>
                </div>
                <div className="modal-fields-row">
                  <AccountField label="Tipo de cuenta"><select className="modal-select" value={form.accountType} onChange={e=>setForm(p=>({...p,accountType:e.target.value}))}><option value="ahorro">Ahorro</option><option value="corriente">Corriente</option><option value="nomina">Nómina</option></select></AccountField>
                  <AccountField label="Balance inicial"><input className="modal-input" type="number" placeholder="0.00" value={form.balance} onChange={e=>setForm(p=>({...p,balance:e.target.value}))}/></AccountField>
                </div>
                <div className="modal-fields-row">
                  <AccountField label="Límite retiro diario"><input className="modal-input" type="number" placeholder="1000" value={form.dailyWithdrawalLimit} onChange={e=>setForm(p=>({...p,dailyWithdrawalLimit:e.target.value}))}/></AccountField>
                  <AccountField label="Tasa interés anual (%)"><input className="modal-input" type="number" placeholder="4.5" value={form.annualInterestRate} onChange={e=>setForm(p=>({...p,annualInterestRate:e.target.value}))}/></AccountField>
                </div>
                <div className="modal-fields-row">
                  <AccountField label="Fecha apertura"><input className="modal-input" type="date" value={form.openingDate} onChange={e=>setForm(p=>({...p,openingDate:e.target.value}))}/></AccountField>
                  <AccountField label="Estado inicial"><select className="modal-select" value={form.status} onChange={e=>setForm(p=>({...p,status:e.target.value}))}><option value="activa">Activa</option><option value="inactiva">Inactiva</option></select></AccountField>
                </div>
              </>)}
              {editItem && (<>
                <div style={{background:'rgba(200,169,81,0.05)',border:'1px solid rgba(200,169,81,0.12)',borderRadius:8,padding:'.85rem 1rem',marginBottom:'.5rem',fontSize:'.82rem',color:'var(--gold-bright)'}}>Cuenta: <strong style={{fontFamily:'monospace'}}>{editItem.accountNumber}</strong></div>
                <div className="modal-fields-row">
                  <AccountField label="Nombre titular"><input className="modal-input" placeholder="Juan" value={form.name} onChange={e=>setForm(p=>({...p,name:e.target.value}))}/></AccountField>
                  <AccountField label="Ocupación"><input className="modal-input" placeholder="Desarrollador" value={form.jobName} onChange={e=>setForm(p=>({...p,jobName:e.target.value}))}/></AccountField>
                </div>
                <AccountField label="Dirección"><input className="modal-input" placeholder="Zona 1, Ciudad de Guatemala" value={form.address} onChange={e=>setForm(p=>({...p,address:e.target.value}))}/></AccountField>
                <div className="modal-fields-row">
                  <AccountField label="Ingreso mensual (Q)"><input className="modal-input" type="number" placeholder="5000" value={form.monthlyIncome} onChange={e=>setForm(p=>({...p,monthlyIncome:e.target.value}))}/></AccountField>
                  <AccountField label="Teléfono (8 dígitos)"><input className="modal-input" placeholder="55123456" maxLength={8} value={form.phone} onChange={e=>setForm(p=>({...p,phone:e.target.value.replace(/\D/g,'')}))}/></AccountField>
                </div>
            ...
            <div className="modal-footer">
              ...
              <button className="btn-save" onClick={handleSave} disabled={saving}>{saving ? <span className="spin"/> : editItem ? 'Actualizar' : 'Crear cuenta'}</button>
            </div>
          </div>
        </div>
      )}
      ...
      {detailModal && (
        <div className="modal-overlay" onClick={()=>setDetailModal(null)}>
          <div className="modal" style={{maxWidth:500}} onClick={e=>e.stopPropagation()}>
            <div className="modal-header"><span className="modal-title">Detalle de cuenta</span><button className="modal-close" onClick={()=>setDetailModal(null)}>✕</button></div>
            <div className="modal-body">
              {[
      ...
      <ConfirmModal open={!!confirm} title="Eliminar cuenta" message={`¿Eliminar la cuenta ${confirm?.accountNumber}? Esta acción no se puede deshacer.`} onConfirm={handleDelete} onCancel={()=>setConfirm(null)}/>
    </div>
  );
};

/* ══════════════════════════════════
   SECCIÓN: Monedas (conservada de compañeros)
══════════════════════════════════ */
const CoinsSection = () => {
  ...
  const handleCreate = async () => {
    setSaving(true);
    try { await createCoin({ ...form, exchangeRate:Number(form.exchangeRate) }); showSuccess('Moneda creada'); setModal(false); reload(); }
  ...
  return (
    <div>
      <div className="page-header">
        ...
        <button className="btn-add" onClick={()=>setModal(true)}><svg viewBox="0 0 24 24" fill="none" width="14" height="14"><path d="M12 5v14M5 12h14" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/></svg>Nueva moneda</button>
      </div>
      <div className="table-card">
              ...
              return (
                <tr key={i}>
                  ...
                  <td><div className="action-btns">
                    <button className="btn-icon danger" onClick={()=>setConfirm({id,label:c.code||c.Code})}><svg viewBox="0 0 24 24" fill="none" width="13" height="13"><path d="M3 6h18M19 6l-1 14H6L5 6M10 11v6M14 11v6M9 6V4h6v2" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg></button>
                  </div></td>
                </tr>
              );
            })}
...
/* ══════════════════════════════════
   SECCIÓN: Cuentas Bloqueadas (conservada de compañeros — intacta)
══════════════════════════════════ */
...
const LocksSection = () => {
  ...
  const emptyForm = { accountId:'', userId:'', lockReason:'seguridad', description:'', lockDate:'', unlockDate:'', lockedBy:'', unlockedBy:'', status:'bloqueado' };
  const [form, setForm] = useState(emptyForm);

  useEffect(() => { if (data.length > 0) setLocalData(data); }, [data]);

  const filtered = localData.filter(l =>
    `${l.accountId||''} ${l.userId||''} ${l.lockReason||''} ${l.status||''}`.toLowerCase().includes(search.toLowerCase())
  );

  const openCreate = () => { setForm(emptyForm); setEditItem(null); setModal(true); };
  const openEdit = (l) => {
    setForm({ accountId: l.accountId||'', userId: l.userId||'', lockReason: l.lockReason||'seguridad', description: l.description||'', lockDate: l.lockDate?l.lockDate.slice(0,16):'', unlockDate: l.unlockDate?l.unlockDate.slice(0,16):'', lockedBy: l.lockedBy||'', status: l.status||'bloqueado', unlockedBy: l.unlockedBy||'' });
    setEditItem(l); setModal(true);
  };

  const handleSave = async () => {
    if (!form.accountId || !form.userId) { showError('N° de cuenta e ID de usuario son obligatorios'); return; }
    setSaving(true);
    try {
      const payload = { accountId: form.accountId.trim(), userId: form.userId.trim(), lockReason: form.lockReason, description: form.description, lockDate: form.lockDate||new Date().toISOString(), unlockDate: form.unlockDate?new Date(form.unlockDate).toISOString():null, lockedBy: form.lockedBy||form.userId, unlockedBy: form.status==='desbloqueado'?(form.lockedBy||user?.id||form.userId):undefined, status: form.status };
      Object.keys(payload).forEach(k => { if (payload[k] === undefined) delete payload[k]; });
      if (editItem) {
        const id = editItem._id || editItem.id;
        await updateAccountLock(id, payload);
        if (form.status === 'desbloqueado') { try { await toggleAccountStatus(payload.accountId, 'activa'); } catch {} }
      ...
      setModal(false); clearDataCache();
    } catch (e) { showError(e?.response?.data?.message || e?.response?.data?.error || 'Error al guardar'); }
    finally { setSaving(false); }
  };

  const handleDelete = async () => {
    const id = confirm._id||confirm.id;
    try { await deleteAccountLock(id); try { await toggleAccountStatus(confirm.accountId, 'activa'); } catch {} setLocalData(prev => prev.filter(l => (l._id||l.id) !== id)); showSuccess('Bloqueo eliminado — cuenta desbloqueada'); }
    catch(e) { showError(e?.response?.data?.message || 'Error al eliminar'); }
    setConfirm(null);
  };
  ...
  return (
    <div>
      <div className="page-header">
        <div><h1 className="page-title">Cuentas Bloqueadas</h1><p className="page-subtitle">Gestión de bloqueos — al crear un bloqueo la cuenta queda bloqueada automáticamente</p></div>
        <button className="btn-add" onClick={openCreate}><svg viewBox="0 0 24 24" fill="none" width="14" height="14"><path d="M12 5v14M5 12h14" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/></svg>Bloquear cuenta</button>
      </div>
      ...
      <div className="stats-grid" style={{marginBottom:'1.25rem'}}>
        {[
          ...
          <div key={i} className="stat-card"><div className="stat-card-value" style={{fontSize:'1.4rem'}}>{s.value}</div><div className="stat-card-label">{s.label}</div></div>
        ))}
      </div>
      ...
      <div className="table-card">
        ...
        <table className="data-table">
          <thead><tr><th>Cuenta</th><th>Usuario</th><th>Motivo</th><th>Descripción</th><th>Bloqueado por</th><th>Fecha bloqueo</th><th>Estado</th><th>Acciones</th></tr></thead>
          <tbody>
            {loading && localData.length===0 ? <LoadingRows cols={8}/> : filtered.map((l,i) => (
              <tr key={l._id||l.id||i}>
                ...
                <td>
                  <div className="action-btns">
                    <button className="btn-icon" title="Editar bloqueo" onClick={()=>openEdit(l)}><svg viewBox="0 0 24 24" fill="none" width="13" height="13"><path d="M11 4H4a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2v-7" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/><path d="M18.5 2.5a2.121 2.121 0 013 3L12 15l-4 1 1-4 9.5-9.5z" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/></svg></button>
                    <button className="btn-icon danger" title="Eliminar bloqueo" onClick={()=>setConfirm(l)}><svg viewBox="0 0 24 24" fill="none" width="13" height="13"><path d="M3 6h18M19 6l-1 14H6L5 6M10 11v6M14 11v6M9 6V4h6v2" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg></button>
                  </div>
                </td>
              </tr>
            ))}
      ...
      {modal && (
        <div className="modal-overlay" onClick={()=>setModal(false)}>
          <div className="modal" style={{maxWidth:540}} onClick={e=>e.stopPropagation()}>
            <div className="modal-header"><span className="modal-title">{editItem ? 'Editar bloqueo' : 'Bloquear cuenta'}</span><button className="modal-close" onClick={()=>setModal(false)}>✕</button></div>
            <div className="modal-body">
              <div className="modal-fields-row">
                <LockField label="N° de cuenta *"><input className="modal-input" placeholder="ACC-000-0000" value={form.accountId} onChange={e=>setForm(p=>({...p,accountId:e.target.value}))}/></LockField>
                <LockField label="ID de usuario *"><input className="modal-input" placeholder="usr_XXXX" value={form.userId} onChange={e=>setForm(p=>({...p,userId:e.target.value}))}/></LockField>
              </div>
              <div className="modal-fields-row">
                <LockField label="Motivo del bloqueo"><select className="modal-select" value={form.lockReason} onChange={e=>setForm(p=>({...p,lockReason:e.target.value}))}>{lockReasons.map(r=><option key={r} value={r}>{r.replace(/_/g,' ')}</option>)}</select></LockField>
                <LockField label="Estado"><select className="modal-select" value={form.status} onChange={e=>setForm(p=>({...p,status:e.target.value}))}><option value="bloqueado">Bloqueado</option><option value="desbloqueado">Desbloqueado</option></select></LockField>
              </div>
              <LockField label="Descripción"><input className="modal-input" placeholder="Describe el motivo del bloqueo..." value={form.description} onChange={e=>setForm(p=>({...p,description:e.target.value}))}/></LockField>
              <div className="modal-fields-row">
                <LockField label="Fecha de bloqueo"><input className="modal-input" type="datetime-local" value={form.lockDate} onChange={e=>setForm(p=>({...p,lockDate:e.target.value}))}/></LockField>
                <LockField label="Fecha de desbloqueo"><input className="modal-input" type="datetime-local" value={form.unlockDate} onChange={e=>setForm(p=>({...p,unlockDate:e.target.value}))}/></LockField>
              </div>
              {form.status === 'desbloqueado' && (<LockField label="Desbloqueado por (ID admin)"><input className="modal-input" placeholder="usr_XXXX" value={form.unlockedBy} onChange={e=>setForm(p=>({...p,unlockedBy:e.target.value}))}/></LockField>)}
              <div style={{background:'rgba(224,92,92,0.06)',border:'1px solid rgba(224,92,92,0.15)',borderRadius:8,padding:'.85rem 1rem',display:'flex',gap:'.6rem',alignItems:'flex-start'}}>
                ...
                <p style={{fontSize:'.78rem',color:'rgba(224,92,92,0.8)',lineHeight:1.5,margin:0}}>{editItem?'Al cambiar el estado a "Desbloqueado" la cuenta quedará activa nuevamente.':'Al crear este bloqueo la cuenta será bloqueada automáticamente.'}</p>
              </div>
            </div>
            <div className="modal-footer">
              <button className="btn-cancel" onClick={()=>setModal(false)}>Cancelar</button>
              <button className="btn-save" onClick={handleSave} disabled={saving} style={!editItem?{background:'linear-gradient(135deg,#c0392b,#e05c5c)'}:{}}>{saving?<span className="spin"/>:editItem?'Actualizar bloqueo':'Bloquear cuenta'}</button>
            </div>
          </div>
        </div>
      )}
      <ConfirmModal open={!!confirm} title="Eliminar bloqueo" message={`¿Eliminar el bloqueo de la cuenta ${confirm?.accountId}? La cuenta quedará activa nuevamente.`} onConfirm={handleDelete} onCancel={()=>setConfirm(null)}/>
    </div>
  );
};

/* ══════════════════════════════════
   SECCIÓN: Servicios (conservada de compañeros)
══════════════════════════════════ */
...
/* ══════════════════════════════════
   SECCIÓN: Depósitos (conservada y limpiada)
══════════════════════════════════ */
  ...
  return (
    <div>
      <div className="page-header">
        ...
        <button className="btn-add" onClick={()=>setModal(true)}><svg viewBox="0 0 24 24" fill="none" width="14" height="14"><path d="M12 5v14M5 12h14" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/></svg>Nuevo depósito</button>
      </div>
      <div className="table-card">
        <div style={{padding:'3rem',textAlign:'center',color:'var(--muted)'}}>
          <svg viewBox="0 0 24 24" fill="none" width="40" height="40" style={{opacity:.2,marginBottom:'1rem'}}><path d="M12 5v14M5 12l7 7 7-7" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg>
...
/* ══════════════════════════════════
   SECCIÓN: Retiros (conservada y limpiada — sin require/import dinámico)
══════════════════════════════════ */
  ...
  return (
    <div>
      <div className="page-header">
        ...
        <button className="btn-add" onClick={()=>setModal(true)}><svg viewBox="0 0 24 24" fill="none" width="14" height="14"><path d="M12 5v14M5 12h14" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/></svg>Nuevo retiro</button>
      </div>
      <div className="table-card">
        <div style={{padding:'3rem',textAlign:'center',color:'var(--muted)'}}>
          <svg viewBox="0 0 24 24" fill="none" width="40" height="40" style={{opacity:.2,marginBottom:'1rem'}}><path d="M12 19V5M5 12l7-7 7 7" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg>
...
/* ══════════════════════════════════
   SECCIÓN: Estados de cuenta (conservada)
══════════════════════════════════ */
...
const SECTIONS = {
  ...
  cards:        CardsPageAdmin,        // ← CRUD completo de tarjetas
  transactions: TransactionsPageAdmin, // ← ver todas las transacciones
  loans:        LoansPageAdmin,        // ← gestión completa de préstamos
  coins:        CoinsSection,
  locks:        LocksSection,
