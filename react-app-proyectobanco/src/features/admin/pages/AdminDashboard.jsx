...
const StatementsSection = () => {
  ...
  return (
    <div>
      ...
      <div className="stats-grid" style={{marginBottom:'1.25rem'}}>
        {[
          { label:'Total generados', value: loading?'...':data.length },
          { label:'Este mes', value: loading?'...':data.filter(s=>{
            const d = new Date(s.createdAt);
            const now = new Date();
            return d.getMonth()===now.getMonth() && d.getFullYear()===now.getFullYear();
          }).length },
          { label:'Total depósitos', value: loading?'...':'Q '+fmt(data.reduce((s,x)=>s+Number(x.totalDeposits||0),0)) },
          { label:'Total retiros', value: loading?'...':'Q '+fmt(data.reduce((s,x)=>s+Number(x.totalWithdrawals||0),0)) },
        ].map((s,i)=>(
          <div key={i} className="stat-card">
            <div className="stat-card-value" style={{fontSize:'1.2rem'}}>{s.value}</div>
            <div className="stat-card-label">{s.label}</div>
      ...
      <div className="table-card">
        ...
        <table className="data-table">
          ...
          <tbody>
            {loading ? <LoadingRows cols={10}/> : filtered.map((s,i)=>(
              <tr key={s._id||i}>
